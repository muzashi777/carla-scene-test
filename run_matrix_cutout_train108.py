# -*- coding: utf-8 -*-
"""
Iterate through all TEST MATRIX cases for the Cut-out scenario on train108 (headless, no display by default).
Writes results/cutout108_matrix_<TEST_MODE>_<stamp>.csv + prints CPEIM summary.

Before running: load the train108 scene in CARLA.
Usage:
    python run_matrix_cutout_train108.py                   # original controllers, no viz
    TEST_MODE=latency python run_matrix_cutout_train108.py
    MATRIX_VIZ=1 python run_matrix_cutout_train108.py      # opt-in display
"""
...
def main():
    ...
    cases = build_cases()
    print(f"[MATRIX/cutout_train108] {len(cases)} cases/controller × {len(matrix_runs)} runs "
    ...
    prefix = getattr(cfg, "RESULTS_PREFIX", "cutout108_matrix")
    ...
    print("\n" + "=" * 72)
    print("Summary per controller (Cut-out train108 scenario):")
