# -*- coding: utf-8 -*-
"""
Iterate through all TEST MATRIX cases for the Junction Cut-in scenario (headless, no display by default).
Writes results/junction_matrix_<TEST_MODE>_<stamp>.csv + prints CPEIM summary.

Before running: load the train105 scene in CARLA.
Usage:
    python run_matrix_junction_cutin.py                    # original controllers, no viz
    TEST_MODE=latency python run_matrix_junction_cutin.py
    MATRIX_VIZ=1 python run_matrix_junction_cutin.py       # opt-in display
"""
...
def build_cases():
    ...
    trigger_d: ego↔intruder distance at which the intruder begins turning right.
    Larger trigger_d → intruder turns earlier → more warning time for AEB.
    """
    cases = []
    for v, td, m in itertools.product(
            cfg.MATRIX["ego_speed_kmh"], cfg.MATRIX["trigger_d"], cfg.MATRIX["mu"]):
        cases.append({
            "ego_speed_kmh": v,
            "trigger_d":     td,
            "mu":            m,
...
def main():
    ...
    cases = build_cases()
    print(f"[MATRIX/junction_cutin] {len(cases)} cases/controller × {len(matrix_runs)} runs "
    ...
    prefix = getattr(cfg, "RESULTS_PREFIX", "junction_matrix")
    ...
    print("\n" + "=" * 72)
    print("Summary per controller (Junction Cut-in scenario):")
